...
class CsvModel extends CI_Model {
	...
	public function import_data($file_path) {
		...
		foreach ($csv_data as $row) {
			$type_maison_id = $this->get_or_create_type_maison($row['type_maison'], $row['description'],$row['surface']);
			$code_travaux_id = $this->get_or_create_code_travaux($row['code_travaux'], $row['type_travaux'],$row['prix_unitaire']);
